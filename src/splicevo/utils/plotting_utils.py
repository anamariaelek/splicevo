...
def plot_splice_site_usage(
    ...
    # Convert timepoints to numeric, handling 'NA' as 0
    ...
    for idx, site_info_dict in enumerate(site_info):
        ...
        for tissue in tissues:
            ...
            for tp_num in timepoints_numeric:
                tp_data = tissue_data[tissue_data['timepoint_numeric'] == tp_num]['sse'].values
            ...
            if timepoint_data:
                ...
                for tp_num, sse_values in timepoint_data.items():
                    ...
                    ax.scatter(x_jitter, sse_values, 
                              color=tissue_colors[tissue], alpha=0.6, s=50)
                ...
                ax.plot(tp_nums, means, 
                       marker='o', label=tissue, color=tissue_colors[tissue], 
        ...
        title_text = f'{chrom}:{genomic_pos}({strand}) | idx:{int(window_idx)} pos:{int(data_pos)} | {site_type_label}'
        ...
        ax.set_xticklabels([int(tp) if tp != 0 else '0' for tp in timepoints_numeric])
        ax.grid(True, alpha=0.3, axis='y')
        
        # Add legend only to first subplot
        if idx == 0:
            ax.legend(title='Tissue', loc='best', framealpha=0.9, fontsize=8)
...
def plot_multiple_windows_comparison(
    analysis_results: List[Dict],
    figsize: tuple = (14, 4),
    max_windows: int = 5
) -> plt.Figure:
    """
    Create subplots comparing SSE usage across multiple windows.
    
    Args:
        analysis_results: List of analysis data dicts
        figsize: Figure size (width, height)
    ...
    """
    results_to_plot = analysis_results[:max_windows]
    n_windows = len(results_to_plot)
    
    fig, axes = plt.subplots(1, n_windows, figsize=figsize)
    if n_windows == 1:
        axes = [axes]
    
    for idx, analysis_result in enumerate(results_to_plot):
        usage_data = analysis_result.get('usage_data')
        if not usage_data:
            continue
        
        ax = axes[idx]
        
        # Collect position and SSE data
        positions = []
        sse_values = []
        site_labels = []
        
        for site_type in ['donors', 'acceptors']:
            for site_data in usage_data.get(site_type, []):
                pos = site_data['position']
                # Get mean SSE across all conditions
                sse_list = [cond['sse'] for cond in site_data.get('conditions', {}).values()]
                if sse_list:
                    positions.append(pos)
                    sse_values.append(np.mean(sse_list))
                    site_labels.append(site_type[0].upper())
        
        if positions:
            # Create scatter plot
            colors = ['red' if label == 'D' else 'blue' for label in site_labels]
            ax.scatter(positions, sse_values, c=colors, s=100, alpha=0.6)
            ax.set_ylim(0, 1)
            ax.set_xlabel('Position', fontsize=10)
            ax.set_ylabel('Mean SSE', fontsize=10)
            ax.set_title(f"{analysis_result['gene_id']}\n{analysis_result['chromosome']}:{analysis_result['start']}-{analysis_result['end']}", 
                        fontsize=10)
        ...
        if idx == 0:
            ...
            legend_elements = [
                Patch(facecolor='red', label='Donor sites'),
                Patch(facecolor='blue', label='Acceptor sites')
            ]
